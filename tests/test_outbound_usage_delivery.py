...
def test_node_outbound_usage_replaces_cumulative_node_batches(monkeypatch):
    ...
    monkeypatch.setattr(node_usage, "xray", fake_xray)
    monkeypatch.setattr(node_usage, "_persist_node_usage_batch", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("db is down")))
...
def test_user_usage_ack_prevents_replay_when_hourly_snapshot_fails(monkeypatch):
    ...
    monkeypatch.setattr(user_usage, "record_user_stats", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("locked")))
